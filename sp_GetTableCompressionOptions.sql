...
--EXEC sp_ms_marksystemobject 'sp_GetTableCompressionOptions' 
...
(
   SELECT s.name AS SchemaName
      ...
      , CASE
         ...
         ELSE 'Heap'
      END AS TableShape
      , FORMAT(p. rows, 'N0') AS NumberOfRows
      ...
      , FORMAT(p.DataPageCount * 8.0 / 1024, 'N0') AS MBData
...
SELECT DISTINCT tc.SchemaName, tc.TableName, tc.IndexName
   ...
   --, tc.MBDataNumeric, DB_NAME() AS DatabaseName
   , tc.IndexId
FROM TableCompression AS tc
...
ORDER BY tc.NumberOfRowsNumeric DESC, tc.SchemaName, tc.TableName, tc.IndexId
